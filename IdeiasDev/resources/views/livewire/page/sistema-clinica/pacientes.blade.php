<div>
    <div class="bg-gray-900 border border-gray-800 p-6 rounded-xl mb-6">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-semibold text-gray-100">Pacientes</h1>
            <button wire:click="openModal" class="px-2 sm:px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition" title="Novo Paciente">
                <i class="fas fa-plus"></i>
                <span class="hidden sm:inline sm:ml-1.5">Novo Paciente</span>
            </button>
        </div>
    </div>
            ...
            <tbody class="divide-y divide-gray-800">
                @forelse ($patients as $p)
                    <tr class="hover:bg-gray-800/50">
                        ...
                        <td class="px-6 py-4">{{ $p->birth_date?->format('d/m/Y') ?? '-' }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="4" class="px-6 py-12 text-gray-500">Nenhum paciente encontrado.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="flex items-center justify-end mt-4">
        <div class="text-xs text-gray-500">
            Mostrando {{ $patients->firstItem() ?? 0 }} a {{ $patients->lastItem() ?? 0 }} de {{ $patients->total() }} pacientes
        </div>
    </div>
    ...
    @if ($showModal)
        <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
            <div class="bg-gray-900 border border-gray-800 rounded-lg p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
                <h3 class="text-lg font-semibold text-white mb-4">{{ $patientId ? 'Editar' : 'Novo' }} Paciente</h3>
