<div>
    <div class="bg-gray-900 border border-gray-800 p-6 rounded-xl mb-6">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-semibold text-gray-100">Listas de Email</h1>
            <button wire:click="openModal" class="px-2 sm:px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition" title="Nova Lista">
                <i class="fas fa-plus"></i>
                <span class="hidden sm:inline sm:ml-1.5">Nova Lista</span>
            </button>
        </div>
    </div>
        ...
        <table class="w-full text-sm text-left text-gray-400">
            <thead class="text-xs uppercase bg-gray-800">
                <tr>
                    ...
                    <th class="px-6 py-3">Assinantes</th>
            ...
            <tbody class="divide-y divide-gray-800">
                @forelse ($listas as $lista)
                    <tr class="hover:bg-gray-800/50">
                        <td class="px-6 py-4 text-white">{{ $lista->name }}</td>
                        <td class="px-6 py-4">{{ $lista->description ? \Illuminate\Support\Str::limit(strip_tags($lista->description), 60) : '-' }}</td>
                        <td class="px-6 py-4">{{ $lista->subscribers()->count() }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-right">
                            <div class="flex items-center justify-end gap-1">
                                <button wire:click="edit({{ $lista->id }})" class="p-1.5 text-blue-500 hover:text-blue-400 hover:bg-blue-500/10 rounded-lg transition" title="Editar">
                                    <i class="fas fa-edit"></i>
                                </button>
                                <button wire:click="confirmDelete({{ $lista->id }})" class="p-1.5 text-red-500 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition" title="Excluir">
                ...
                @empty
                    <tr>
                        <td colspan="4" class="px-6 py-12 text-gray-500">Nenhuma lista encontrada.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="flex items-center justify-end mt-4">
        <div class="text-xs text-gray-500">
            Mostrando {{ $listas->firstItem() ?? 0 }} a {{ $listas->lastItem() ?? 0 }} de {{ $listas->total() }} listas
        </div>
    </div>
    ...
    @if ($showModal)
        <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
            <div class="bg-gray-900 border border-gray-800 rounded-lg p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
                <h3 class="text-lg font-semibold text-white mb-4">{{ $listaId ? 'Editar' : 'Nova' }} Lista</h3>
