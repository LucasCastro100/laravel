<div>
    <div class="bg-gray-900 border border-gray-800 p-6 rounded-xl mb-6">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-semibold text-gray-100">Mesas</h1>
            <button wire:click="openModal" class="px-2 sm:px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition" title="Nova Mesa">
                <i class="fas fa-plus"></i>
                <span class="hidden sm:inline sm:ml-1.5">Nova Mesa</span>
            </button>
        </div>
    </div>
            ...
            <tbody class="divide-y divide-gray-800">
                @forelse ($mesas as $m)
                    <tr class="hover:bg-gray-800/50">
                        ...
                        <td class="px-6 py-4">{{ $m->seats }}</td>
                        <td class="px-6 py-4">
                            <span class="px-2 py-1 text-xs rounded-full {{ $m->status === 'ocupada' ? 'bg-red-900 text-red-400' : 'bg-green-900 text-green-400' }}">
                ...
                @empty
                    <tr>
                        <td colspan="4" class="px-6 py-12 text-gray-500">Nenhuma mesa encontrada.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="flex items-center justify-end mt-4">
        <div class="text-xs text-gray-500">
            Mostrando {{ $mesas->firstItem() ?? 0 }} a {{ $mesas->lastItem() ?? 0 }} de {{ $mesas->total() }} mesas
        </div>
    </div>
    ...
    @if ($showModal)
        <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
            <div class="bg-gray-900 border border-gray-800 rounded-lg p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
                <h3 class="text-lg font-semibold text-white mb-4">{{ $mesaId ? 'Editar' : 'Nova' }} Mesa</h3>
                <form wire:submit="save" class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm text-gray-300 mb-1">Lugares</label>
                            <input wire:model="seats" type="number" min="1" class="w-full bg-gray-800 border border-gray-700 text-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            @error('seats') <span class="text-red-400 text-xs mt-1 block">{{ $message }}</span> @enderror
                        </div>
                        <div>
