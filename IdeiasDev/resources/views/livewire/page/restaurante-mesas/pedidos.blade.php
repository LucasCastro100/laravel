            ...
            <tbody class="divide-y divide-gray-800">
                @forelse ($pedidos as $p)
                    <tr class="hover:bg-gray-800/50">
                        <td class="px-6 py-4 text-white">{{ $p->table?->name ?? '-' }}</td>
                        <td class="px-6 py-4">{{ \Illuminate\Support\Str::limit(strip_tags($p->items_summary ?? ''), 40) ?: '-' }}</td>
                        <td class="px-6 py-4 font-medium text-green-400">R$ {{ number_format($p->total, 2, ',', '.') }}</td>
                        <td class="px-6 py-4">
                            <span class="px-2 py-1 text-xs rounded-full {{ $p->status === 'aberto' ? 'bg-yellow-900 text-yellow-400' : 'bg-green-900 text-green-400' }}">
        ...
        <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
            <div class="bg-gray-900 border border-gray-800 rounded-lg p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
                ...
                <form wire:submit="save" class="space-y-4">
                    <div>
                        <label class="block text-sm text-gray-300 mb-1">Mesa</label>
                        <x-searchable-select model="table_id" :options="$mesas->map(fn($m) => ['value' => $m->id, 'label' => $m->name])->toArray()" placeholder="Selecione a mesa" :initial="$table_id" />
                        @error('table_id') <span class="text-red-400 text-xs mt-1 block">{{ $message }}</span> @enderror
                    </div>
                    <x-rich-text-editor model="items_summary" label="Itens" />
