<div>
    <div class="bg-gray-900 border border-gray-800 p-6 rounded-xl mb-6">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-semibold text-gray-100">Turmas</h1>
            <button wire:click="openModal" class="px-2 sm:px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition" title="Nova Turma">
                <i class="fas fa-plus"></i>
                <span class="hidden sm:inline sm:ml-1.5">Nova Turma</span>
            </button>
        </div>
    </div>
    ...
    <div class="bg-gray-900 border border-gray-800 rounded-lg p-4 mb-6">
        <div class="flex items-center gap-4 flex-wrap">
            <div class="flex-1 min-w-[200px]">
                <input wire:model.live.debounce.400ms="search" type="text" placeholder="Buscar por nome ou professor..." class="w-full bg-gray-800 border border-gray-700 text-gray-200 rounded px-3 py-2 text-sm">
            </div>
        ...
        <table class="w-full text-sm text-left text-gray-400">
            <thead class="text-xs uppercase bg-gray-800">
                <tr>
                    ...
                    <th class="px-6 py-3">Professor(a)</th>
            ...
            <tbody class="divide-y divide-gray-800">
                @forelse ($turmas as $t)
                    <tr class="hover:bg-gray-800/50">
                        ...
                        <td class="px-6 py-4">{{ $t->teacher_name ?? '-' }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="4" class="px-6 py-12 text-gray-500">Nenhuma turma encontrada.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="flex items-center justify-end mt-4">
        <div class="text-xs text-gray-500">
            Mostrando {{ $turmas->firstItem() ?? 0 }} a {{ $turmas->lastItem() ?? 0 }} de {{ $turmas->total() }} turmas
        </div>
    </div>
    ...
    @if ($showModal)
        <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
            <div class="bg-gray-900 border border-gray-800 rounded-lg p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
                <h3 class="text-lg font-semibold text-white mb-4">{{ $turmaId ? 'Editar' : 'Nova' }} Turma</h3>
                <form wire:submit="save" class="space-y-4">
                    ...
                    <div>
                        <label class="block text-sm text-gray-300 mb-1">Professor(a)</label>
                        <input wire:model="teacher_name" type="text" class="w-full bg-gray-800 border border-gray-700 text-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        @error('teacher_name') <span class="text-red-400 text-xs mt-1 block">{{ $message }}</span> @enderror
                    </div>
                    <div>
                        <label class="block text-sm text-gray-300 mb-1">Turno</label>
                        <select wire:model="shift" class="w-full bg-gray-800 border border-gray-700 text-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        ...
                        @error('shift') <span class="text-red-400 text-xs mt-1 block">{{ $message }}</span> @enderror
                    </div>
