<div>
    <div class="bg-gray-900 border border-gray-800 p-6 rounded-xl mb-6">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-semibold text-gray-100">Alunos</h1>
            <button wire:click="openModal" class="px-2 sm:px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition" title="Novo Aluno">
                <i class="fas fa-plus"></i>
                <span class="hidden sm:inline sm:ml-1.5">Novo Aluno</span>
            </button>
        </div>
    </div>
    ...
    <div class="bg-gray-900 border border-gray-800 rounded-lg p-4 mb-6">
        <div class="flex items-center gap-4 flex-wrap">
            <div class="flex-1 min-w-[200px]">
                <input wire:model.live.debounce.400ms="search" type="text" placeholder="Buscar por nome ou responsável..." class="w-full bg-gray-800 border border-gray-700 text-gray-200 rounded px-3 py-2 text-sm">
            </div>
            ...
            <tbody class="divide-y divide-gray-800">
                @forelse ($alunos as $a)
                    <tr class="hover:bg-gray-800/50">
                        ...
                        <td class="px-6 py-4">{{ $a->turma?->name ?? '-' }}</td>
                        <td class="px-6 py-4">{{ $a->guardian_name ?? '-' }}</td>
                        <td class="px-6 py-4">{{ $a->guardian_phone ?? '-' }}</td>
                        <td class="px-6 py-4">
                            <span class="px-2 py-1 text-xs rounded-full {{ $a->active ? 'bg-green-900 text-green-400' : 'bg-gray-800 text-gray-400' }}">
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-gray-500">Nenhum aluno encontrado.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    @if ($showModal)
        <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
            <div class="bg-gray-900 border border-gray-800 rounded-lg p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
                <h3 class="text-lg font-semibold text-white mb-4">{{ $alunoId ? 'Editar' : 'Novo' }} Aluno</h3>
                <form wire:submit="save" class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm text-gray-300 mb-1">Turma</label>
                            <x-searchable-select model="class_id" :options="$turmas->map(fn($t) => ['value' => $t->id, 'label' => $t->name])->toArray()" placeholder="Selecione a turma" :initial="$class_id" />
                            @error('class_id') <span class="text-red-400 text-xs mt-1 block">{{ $message }}</span> @enderror
                        </div>
                        <div>
                            <label class="block text-sm text-gray-300 mb-1">Data de Nascimento</label>
                            <input wire:model="birth_date" type="date" class="w-full bg-gray-800 border border-gray-700 text-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            @error('birth_date') <span class="text-red-400 text-xs mt-1 block">{{ $message }}</span> @enderror
                        </div>
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm text-gray-300 mb-1">Nome do Responsável</label>
                            <input wire:model="guardian_name" type="text" class="w-full bg-gray-800 border border-gray-700 text-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            @error('guardian_name') <span class="text-red-400 text-xs mt-1 block">{{ $message }}</span> @enderror
                        </div>
                        <div>
                            <label class="block text-sm text-gray-300 mb-1">Telefone do Responsável</label>
                            <input wire:model="guardian_phone" type="text" class="w-full bg-gray-800 border border-gray-700 text-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            @error('guardian_phone') <span class="text-red-400 text-xs mt-1 block">{{ $message }}</span> @enderror
                        </div>
                    </div>
                    <div>
