<div>
    <div class="bg-gray-900 border border-gray-800 p-6 rounded-xl mb-6">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-semibold text-gray-100">Motoristas</h1>
            <button wire:click="openModal" class="px-2 sm:px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition" title="Novo Motorista">
                <i class="fas fa-plus"></i>
                <span class="hidden sm:inline sm:ml-1.5">Novo Motorista</span>
            </button>
        </div>
    </div>
            ...
            <tbody class="divide-y divide-gray-800">
                @forelse ($drivers as $d)
                    <tr class="hover:bg-gray-800/50">
                        ...
                        <td class="px-6 py-4">{{ $d->license_no ?? '-' }}</td>
                        <td class="px-6 py-4 capitalize">{{ $d->vehicle_category }}</td>
                        <td class="px-6 py-4">
                            <span class="px-2 py-1 text-xs rounded-full {{ $d->status === 'ativo' ? 'bg-green-900 text-green-400' : 'bg-gray-800 text-gray-400' }}">
                                {{ $d->status === 'ativo' ? 'Ativo' : 'Inativo' }}
                            </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-right">
                            <div class="flex items-center justify-end gap-1">
                                <button wire:click="edit({{ $d->id }})" class="p-1.5 text-blue-500 hover:text-blue-400 hover:bg-blue-500/10 rounded-lg transition" title="Editar">
                                    <i class="fas fa-edit"></i>
                                </button>
                                <button wire:click="confirmDelete({{ $d->id }})" class="p-1.5 text-red-500 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition" title="Excluir">
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-gray-500">Nenhum motorista encontrado.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="flex items-center justify-end mt-4">
        <div class="text-xs text-gray-500">
            Mostrando {{ $drivers->firstItem() ?? 0 }} a {{ $drivers->lastItem() ?? 0 }} de {{ $drivers->total() }} motoristas
        </div>
    </div>
    ...
    @if ($showModal)
        <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
            <div class="bg-gray-900 border border-gray-800 rounded-lg p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
                <h3 class="text-lg font-semibold text-white mb-4">{{ $driverId ? 'Editar' : 'Novo' }} Motorista</h3>
                <form wire:submit="save" class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="block text-sm text-gray-300 mb-1">CNH</label>
                            <input wire:model="license_no" type="text" class="w-full bg-gray-800 border border-gray-700 text-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            @error('license_no') <span class="text-red-400 text-xs mt-1 block">{{ $message }}</span> @enderror
                        </div>
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm text-gray-300 mb-1">Categoria do Veículo</label>
                            <select wire:model="vehicle_category" class="w-full bg-gray-800 border border-gray-700 text-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                                <option value="sedan">Sedan</option>
                                <option value="suv">SUV</option>
                                <option value="hatch">Hatch</option>
                                <option value="moto">Moto</option>
                            </select>
                            @error('vehicle_category') <span class="text-red-400 text-xs mt-1 block">{{ $message }}</span> @enderror
                        </div>
                        <div>
