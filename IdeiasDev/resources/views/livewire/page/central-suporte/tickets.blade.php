<div>
    <div class="bg-gray-900 border border-gray-800 p-6 rounded-xl mb-6">
        <div class="flex items-center justify-between">
            ...
            <button wire:click="openModal" class="px-2 sm:px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition" title="Novo Ticket">
                <i class="fas fa-plus"></i>
                <span class="hidden sm:inline sm:ml-1.5">Novo Ticket</span>
            </button>
        </div>
    </div>
    ...
    <div class="bg-gray-900 border border-gray-800 rounded-lg p-4 mb-6">
        <div class="flex items-center gap-4 flex-wrap">
            <div class="flex-1 min-w-[200px]">
                <input wire:model.live.debounce.300ms="search" type="text" placeholder="Buscar por assunto..." class="w-full bg-gray-800 border border-gray-700 text-gray-200 rounded px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
            </div>
            ...
            <tbody class="divide-y divide-gray-800">
                @forelse ($tickets as $t)
                    <tr class="hover:bg-gray-800/50">
                        ...
                        <td class="px-6 py-4">{{ $t->department?->name ?? '-' }}</td>
                        <td class="px-6 py-4">
                            <span class="px-2 py-1 text-xs rounded-full {{ $t->priority === 'alta' ? 'bg-red-900 text-red-400' : ($t->priority === 'media' ? 'bg-yellow-900 text-yellow-400' : 'bg-green-900 text-green-400') }}">
                                {{ ucfirst($t->priority) }}
                            </span>
                        </td>
                        <td class="px-6 py-4">
                            <span class="px-2 py-1 text-xs rounded-full {{ $t->status === 'aberto' ? 'bg-red-900 text-red-400' : ($t->status === 'pendente' ? 'bg-yellow-900 text-yellow-400' : 'bg-green-900 text-green-400') }}">
                                {{ ucfirst($t->status) }}
                            </span>
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-6 py-12 text-gray-500">Nenhum ticket encontrado.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    @if ($showModal)
        <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
            <div class="bg-gray-900 border border-gray-800 rounded-lg p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
                <h3 class="text-lg font-semibold text-white mb-4">{{ $ticketId ? 'Editar' : 'Novo' }} Ticket</h3>
                <form wire:submit="save" class="space-y-4">
                    ...
                    <div>
                        <label class="block text-sm text-gray-300 mb-1">Departamento</label>
                        <x-searchable-select model="department_id" :options="$departamentos->map(fn($d) => ['value' => $d->id, 'label' => $d->name])->toArray()" placeholder="Selecione o departamento" :initial="$department_id" />
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm text-gray-300 mb-1">Prioridade</label>
                            <select wire:model="priority" class="w-full bg-gray-800 border border-gray-700 text-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
