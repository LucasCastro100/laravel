<div>
    <div class="bg-gray-900 border border-gray-800 p-6 rounded-xl mb-6">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-semibold text-gray-100">Pastas</h1>
            <button wire:click="openModal" class="px-2 sm:px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition" title="Nova Pasta">
                <i class="fas fa-plus"></i>
                <span class="hidden sm:inline sm:ml-1.5">Nova Pasta</span>
            </button>
        </div>
    </div>
        ...
        <table class="w-full text-sm text-left text-gray-400">
            <thead class="text-xs uppercase bg-gray-800">
                <tr>
                    ...
                    <th class="px-6 py-3">Pasta Pai</th>
            ...
            <tbody class="divide-y divide-gray-800">
                @forelse ($folders as $f)
                    <tr class="hover:bg-gray-800/50">
                        <td class="px-6 py-4 text-white"><i class="fas fa-folder text-yellow-400 mr-2"></i>{{ $f->name }}</td>
                        <td class="px-6 py-4">{{ $f->parent?->name ?? '-' }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="3" class="px-6 py-12 text-gray-500">Nenhuma pasta encontrada.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="flex items-center justify-end mt-4">
        <div class="text-xs text-gray-500">
            Mostrando {{ $folders->firstItem() ?? 0 }} a {{ $folders->lastItem() ?? 0 }} de {{ $folders->total() }} pastas
        </div>
    </div>
    ...
    @if ($showModal)
        <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
            <div class="bg-gray-900 border border-gray-800 rounded-lg p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
                <h3 class="text-lg font-semibold text-white mb-4">{{ $folderId ? 'Editar' : 'Nova' }} Pasta</h3>
                <form wire:submit="save" class="space-y-4">
                    ...
                    <div>
                        <label class="block text-sm text-gray-300 mb-1">Pasta Pai</label>
                        <x-searchable-select model="parent_id" :options="$availableParents->where('id', '!=', $folderId)->map(fn($f) => ['value' => $f->id, 'label' => $f->name])->values()->toArray()" placeholder="Selecione a pasta pai (opcional)" :initial="$parent_id" />
