<div>
    <div class="bg-gray-900 border border-gray-800 p-6 rounded-xl mb-6">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-semibold text-gray-100">Faturas</h1>
            <button wire:click="openModal" class="px-2 sm:px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition" title="Nova Fatura">
                <i class="fas fa-plus"></i>
                <span class="hidden sm:inline sm:ml-1.5">Nova Fatura</span>
            </button>
        </div>
    </div>
            ...
            <tbody class="divide-y divide-gray-800">
                @forelse ($invoices as $i)
                    <tr class="hover:bg-gray-800/50">
                        <td class="px-6 py-4 text-white">{{ $i->client?->name ?? '-' }}</td>
                        <td class="px-6 py-4 font-medium">R$ {{ number_format($i->amount, 2, ',', '.') }}</td>
                        <td class="px-6 py-4">{{ $i->due_date?->format('d/m/Y') ?? '-' }}</td>
                        <td class="px-6 py-4">
                            @php
                                $statusColors = [
                                    'pendente' => 'bg-yellow-900 text-yellow-400',
                                    'pago' => 'bg-green-900 text-green-400',
                                    'vencido' => 'bg-red-900 text-red-400',
                                ];
                            @endphp
                            <span class="px-2 py-1 text-xs rounded-full {{ $statusColors[$i->status] ?? 'bg-gray-800 text-gray-400' }}">
                                {{ ucfirst($i->status) }}
                            </span>
                        </td>
                        <td class="px-6 py-4">{{ $i->paid_at?->format('d/m/Y') ?? '-' }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-right">
                            <div class="flex items-center justify-end gap-1">
                                <button wire:click="edit({{ $i->id }})" class="p-1.5 text-blue-500 hover:text-blue-400 hover:bg-blue-500/10 rounded-lg transition" title="Editar">
                                    <i class="fas fa-edit"></i>
                                </button>
                                <button wire:click="confirmDelete({{ $i->id }})" class="p-1.5 text-red-500 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition" title="Excluir">
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-gray-500">Nenhuma fatura encontrada.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="flex items-center justify-end mt-4">
        <div class="text-xs text-gray-500">
            Mostrando {{ $invoices->firstItem() ?? 0 }} a {{ $invoices->lastItem() ?? 0 }} de {{ $invoices->total() }} faturas
        </div>
    </div>
    ...
    @if ($showModal)
        <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
            <div class="bg-gray-900 border border-gray-800 rounded-lg p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
                <h3 class="text-lg font-semibold text-white mb-4">{{ $invoiceId ? 'Editar' : 'Nova' }} Fatura</h3>
                <form wire:submit="save" class="space-y-4">
                    <div>
                        <label class="block text-sm text-gray-300 mb-1">Cliente</label>
                        <x-searchable-select model="client_id" :options="$clients->map(fn($c) => ['value' => $c->id, 'label' => $c->name])->toArray()" placeholder="Selecione o cliente" :initial="$client_id" />
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="block text-sm text-gray-300 mb-1">Pago em</label>
                            <input wire:model="paid_at" type="date" class="w-full bg-gray-800 border border-gray-700 text-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            @error('paid_at') <span class="text-red-400 text-xs mt-1 block">{{ $message }}</span> @enderror
                        </div>
                    </div>
