<div>
    <div class="bg-gray-900 border border-gray-800 p-6 rounded-xl mb-6">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-semibold text-gray-100">Vendas / NFe</h1>
            ...
            <tbody class="divide-y divide-gray-800">
                @forelse ($sales as $s)
                    <tr class="hover:bg-gray-800/50">
                        <td class="px-6 py-4 text-white">{{ $s->client_name ?? '-' }}</td>
                        <td class="px-6 py-4 font-medium text-green-400">R$ {{ number_format($s->total, 2, ',', '.') }}</td>
                        <td class="px-6 py-4">{{ $s->nfe_number ?? '-' }}</td>
                        <td class="px-6 py-4">{{ $s->sold_at?->format('d/m/Y H:i') ?? '-' }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-right">
                            <div class="flex items-center justify-end gap-1">
                                <button wire:click="edit({{ $s->id }})" class="p-1.5 text-blue-500 hover:text-blue-400 hover:bg-blue-500/10 rounded-lg transition" title="Editar">
                                    <i class="fas fa-edit"></i>
                                </button>
                                <button wire:click="confirmDelete({{ $s->id }})" class="p-1.5 text-red-500 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition" title="Excluir">
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-6 py-12 text-gray-500">Nenhuma venda encontrada.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="flex items-center justify-end mt-4">
        <div class="text-xs text-gray-500">
            Mostrando {{ $sales->firstItem() ?? 0 }} a {{ $sales->lastItem() ?? 0 }} de {{ $sales->total() }} vendas
        </div>
    </div>
    ...
    @if ($showModal)
        <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
            <div class="bg-gray-900 border border-gray-800 rounded-lg p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
                <h3 class="text-lg font-semibold text-white mb-4">{{ $saleId ? 'Editar' : 'Nova' }} Venda</h3>
                <form wire:submit="save" class="space-y-4">
                    <div>
                        <label class="block text-sm text-gray-300 mb-1">Cliente</label>
                        <input wire:model="client_name" type="text" class="w-full bg-gray-800 border border-gray-700 text-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="block text-sm text-gray-300 mb-1">Número da NFe</label>
                            <input wire:model="nfe_number" type="text" class="w-full bg-gray-800 border border-gray-700 text-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            @error('nfe_number') <span class="text-red-400 text-xs mt-1 block">{{ $message }}</span> @enderror
                        </div>
                    </div>
                    <div>
                        <label class="block text-sm text-gray-300 mb-1">Data da Venda</label>
