<div>
    <div class="bg-gray-900 border border-gray-800 p-6 rounded-xl mb-6">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-semibold text-gray-100">Produtos</h1>
            <button wire:click="openModal" class="px-2 sm:px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition" title="Novo Produto">
                <i class="fas fa-plus"></i>
                <span class="hidden sm:inline sm:ml-1.5">Novo Produto</span>
            </button>
        </div>
    </div>
            ...
            <tbody class="divide-y divide-gray-800">
                @forelse ($produtos as $p)
                    <tr class="hover:bg-gray-800/50">
                        <td class="px-6 py-4 text-white">{{ $p->name }}</td>
                        <td class="px-6 py-4">{{ $p->code ?? '-' }}</td>
                        <td class="px-6 py-4 font-medium text-green-400">R$ {{ number_format($p->price, 2, ',', '.') }}</td>
                        <td class="px-6 py-4">{{ $p->cost !== null ? 'R$ ' . number_format($p->cost, 2, ',', '.') : '-' }}</td>
                        <td class="px-6 py-4">
                            <span class="px-2 py-1 text-xs rounded-full {{ $p->stock < 5 ? 'bg-red-900 text-red-400' : 'bg-green-900 text-green-400' }}">
                                {{ $p->stock }}
                            </span>
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-gray-500">Nenhum produto encontrado.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    @if ($showModal)
        <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
            <div class="bg-gray-900 border border-gray-800 rounded-lg p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
                <h3 class="text-lg font-semibold text-white mb-4">{{ $produtoId ? 'Editar' : 'Novo' }} Produto</h3>
                <form wire:submit="save" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input wire:model="code" type="text" class="w-full bg-gray-800 border border-gray-700 text-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="block text-sm text-gray-300 mb-1">Custo (R$)</label>
                            <input wire:model="cost" type="number" step="0.01" min="0" class="w-full bg-gray-800 border border-gray-700 text-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            @error('cost') <span class="text-red-400 text-xs mt-1 block">{{ $message }}</span> @enderror
                        </div>
                    </div>
                    <div>
                        <label class="block text-sm text-gray-300 mb-1">Estoque</label>
                        <input wire:model="stock" type="number" min="0" class="w-full bg-gray-800 border border-gray-700 text-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
