        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6" x-data="{selectedUserImage: {{ json_encode(Auth::user()->image) }}}">
            ...
            <div class="relative mb-4">
                ...
                <label for="dropzone-file"
                    class="flex flex-col items-center justify-center w-full border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-800 hover:bg-gray-700 transition h-[calc(100%-1rem)]">
                    ...
                    <!-- Preview da imagem -->
                    <div id="image-preview-container" class="absolute inset-0 h-100 pt-5 hidden">
        ...
        <div class="flex items-center gap-4">
            ...
            @if (session('status') === 'profile-updated')
                <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 2000)"
                    class="text-sm text-gray-200">{{ __('Salvo.') }}</p>
            @endif
        </div>
    </form>
</section>
