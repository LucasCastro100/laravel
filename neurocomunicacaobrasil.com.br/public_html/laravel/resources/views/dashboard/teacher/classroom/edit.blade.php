<x-app-layout :title="$title">
    <x-slot name="header">
        <x-page-title title="Editar Aula" />
    </x-slot>
    ...
    <div class="py-8">
        <div class="mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex items-center gap-2 text-sm text-gray-500 mb-6">
                ...
                <i class="fa-solid fa-chevron-right text-xs text-gray-700"></i>
                <span class="text-gray-300">Editar Aula</span>
            </div>
            ...
            <div class="bg-gray-900 rounded-2xl border border-gray-800 shadow-xl overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-800 flex items-center gap-3">
                    ...
                    <h2 class="text-base font-semibold text-gray-100">Editar Aula</h2>
                    <span class="ml-auto text-xs text-gray-500 bg-gray-800 px-2.5 py-1 rounded-full">{{ $classroom->module->title }}</span>
                </div>
                ...
                <form method="POST" action="{{ route('classroom.update', $classroom->uuid) }}" class="p-6 space-y-5">
                    @csrf
                    @method('PUT')

                    <div>
                        <label class="block text-sm font-medium text-gray-300 mb-1.5">Nome da Aula <span class="text-red-400">*</span></label>
                        <input type="text" name="title" value="{{ old('title', $classroom->title) }}" required
                    ...
                    <div>
                        ...
                        <input type="url" name="video" value="{{ old('video', $classroom->video) }}" required placeholder="https://www.youtube.com/watch?v=..."
                            class="appearance-none block w-full bg-gray-800 text-gray-100 border border-gray-700 rounded-lg pl-4 pr-10 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition">
                    </div>
                            ...
                            <select name="module_id" required
                                ...
                                @foreach ($course->modules as $module)
                                    <option value="{{ $module->id }}"
                                        {{ old('module_id', $classroom->module_id) == $module->id ? 'selected' : '' }}>
                                        {{ $module->title }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="5"
                            class="block w-full bg-gray-800 text-gray-100 border border-gray-700 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition resize-none">{{ old('description', $classroom->description) }}</textarea>
                    </div>
