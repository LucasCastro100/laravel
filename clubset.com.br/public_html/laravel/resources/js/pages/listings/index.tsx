...
import { Search, MapPin, Info, Package } from 'lucide-react';
...
import { index as listingsIndex, show as listingsShow, create as listingsCreate } from '@/routes/listings';
...
export default function ListingsIndex({
    ...
    const applyFilter = useCallback(
        (key: string, value: string | undefined) => {
            ...
            router.get(listingsIndex().url, params, { preserveState: true, replace: true });
        },
        [filters],
    );
    ...
    const applyFilters = useCallback(
        (updates: Record<string, string | undefined>) => {
            const params: Record<string, unknown> = { ...filters, page: undefined };
                ...
                <div className="flex flex-wrap items-end gap-2">
                    <div className="w-44">
                        <SearchSelect
                            ...
                            onValueChange={(v) => {
                                setSelectedRegion(v);
                                applyFilters({ region: v, state_id: '', municipality_id: '' });
                            }}
                            placeholder="Região"
                            clearable
                        />
                    ...
                    <div className="w-44">
                        <SearchSelect
                            options={filteredStates.map((s) => ({ value: s.id.toString(), label: `${s.uf} - ${s.name}` }))}
                            ...
                            onValueChange={(v) => {
                                ...
                                const state = states.find((s) => s.id.toString() === v);
                                if (state) {
                                    setSelectedRegion(state.region);
                                    applyFilters({ state_id: v, region: state.region });
                                } else {
                                    applyFilter('state_id', v);
                                }
                            }}
                            placeholder={selectedRegion ? 'Estado' : 'Selecione a região'}
                    ...
                    <div className="w-44">
                        <SearchSelect
                            ...
                            onValueChange={(v) => applyFilter('municipality_id', v)}
                            placeholder={!filters.state_id ? 'Selecione o estado' : loadingMunicipalities ? 'Carregando...' : 'Município'}
                            disabled={!filters.state_id || loadingMunicipalities}
                            clearable
                        />
                    </div>
                </div>
                                    ...
                                    <CardContent className="space-y-3">
                                        {listing.imageUrl ? (
                                            <div className="aspect-video overflow-hidden rounded-md">
                                                <img src={listing.imageUrl} alt={listing.title} className="size-full object-cover transition-transform group-hover:scale-105" />
                                            </div>
                                        ) : (
                                        ...
                                        <div className="flex items-start justify-between gap-2">
                                            ...
                                            <Badge variant="secondary" className="shrink-0">{listing.intent}</Badge>
                                        </div>
                ...
                {listings.last_page > 1 && (
                    <div className="flex items-center justify-center gap-2">
                        {Array.from({ length: listings.last_page }, (_, i) => i + 1).map((page) => (
                            <Button
                                key={page}
                                variant={page === listings.current_page ? 'default' : 'outline'}
                                size="sm"
                                onClick={() =>
                                    router.get(
                                        listingsIndex().url,
