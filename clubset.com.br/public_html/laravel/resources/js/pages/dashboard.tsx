...
import { Store, Camera, Handshake, Coins, Clock, CheckCircle2, Eye } from 'lucide-react';
...
export default function Dashboard({ metrics, recentListings, recentMatches }: Props) {
    const metricCards = [
        { label: 'Anúncios', value: metrics.listings.total, icon: Store, detail: `${metrics.listings.active} ativos, ${metrics.listings.pending} pendentes` },
        { label: 'Serviços', value: metrics.services.total, icon: Camera, detail: 'Cadastrados' },
        { label: 'Matches', value: metrics.matches.total, icon: Handshake, detail: `${metrics.matches.completed} concluídos, ${metrics.matches.pending} pendentes` },
        { label: 'Saldo de créditos', value: `R$ ${metrics.credits.balance.toFixed(2)}`, icon: Coins, detail: 'Disponível' },
    ];
                ...
                <div className="grid gap-6 md:grid-cols-2">
                    <div className="space-y-4">
                        <div className="flex items-center justify-between">
                            <h2 className="text-sm font-semibold">Meus anúncios</h2>
                                    ...
                                    >
                                        <div>
                                            ...
                                            <p className="text-xs text-muted-foreground">
                                                {listing.region ?? 'Sem localização'}
                                            </p>
                                        </div>
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center justify-between">
                            <h2 className="text-sm font-semibold">Meus matches</h2>
                            <Button variant="outline" size="sm" asChild>
                        ...
                        {recentMatches.length === 0 ? (
                            ...
                            <div className="space-y-2">
                                {recentMatches.map((match) => (
                                    ...
                                    >
                                        <div>
                                            <p className="text-sm font-medium">{match.listingTitle ?? 'Anúncio'}</p>
                                            <p className="text-xs text-muted-foreground">
                                                {match.providerName ?? 'Usuário'}
                                            </p>
                                        </div>
                                        <div className="flex items-center gap-3">
