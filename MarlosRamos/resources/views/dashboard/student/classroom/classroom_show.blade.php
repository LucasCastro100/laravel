<x-app-layout :title="$title">
    <x-slot name="header">
        <x-page-title
            title="{{ mb_strtoupper($course->title) }} / {{ mb_strtoupper($module_current->title) }} /
            {{ mb_strtoupper($title) }}" />
    </x-slot>
                    ...
                    <div :class="tab === 'comments' ? 'col-span-1 lg:col-span-3' : 'col-span-1 lg:col-span-2'">
                        ...
                        <div class="flex border-b border-gray-200 mb-4 space-x-4">
                            <button @click="tab = 'classroom'"
                                :class="tab === 'classroom' ? 'border-b-2 border-indigo-600 text-indigo-600' : 'text-gray-200'"
                                class="py-2 px-4 font-medium text-sm hover:text-gray-200 hover:border-b-2 hover:border-gray-300">
                                Aula
                            </button>
                            <button @click="tab = 'comments'"
                                :class="tab === 'comments' ? 'border-b-2 border-indigo-600 text-indigo-600' : 'text-gray-200'"
                                class="py-2 px-4 font-medium text-sm hover:text-gray-200 hover:border-b-2 hover:border-gray-300">
                                Comentários
                            </button>
                        </div>
                        ...
                        <div x-show="tab === 'comments'" x-transition>
                            <h3 class="text-xl font-semibold text-gray-100 dark:text-gray-100 mb-4 mt-6">Deixe sua
                            ...
                            <div class="mt-8">
                                @if ($comments->isEmpty())
                                    <p class="text-gray-200 dark:text-gray-400 mt-4">
                                        Nenhum comentário ainda. Seja o primeiro a comentar!
                                    </p>
                                @else
                                    <h4 class="text-lg font-medium text-gray-100 dark:text-gray-100 mb-4">
                                        Comentários:
                                    </h4>

                                    @foreach ($comments as $comment)
                                        <div
                                            class="mt-4 p-4 bg-gray-800 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm">
                                            {{-- Comentário principal --}}
                                            <div class="flex flex-col">
                                                <p class="text-gray-200 dark:text-gray-200">{{ $comment->comment }}</p>
                                                <p class="text-sm text-gray-200 dark:text-gray-400 mt-2 text-right">
                                                    Comentado por: <span
                                                    ...
                                                    @foreach ($comment->replies as $reply)
                                                        <div
                                                            ...
                                                            <p class="text-gray-200 dark:text-gray-200">
                                                                {{ $reply->reply }}</p>
                                                            <p
                                                                class="text-sm text-gray-200 dark:text-gray-400 mt-1 text-right">
                                                                Respondido por: <span
                       ...
                       hover:bg-gray-200 dark:hover:bg-gray-700">
                        ...
                        {{ $key + 1 }}
                                    ...
                                    <div x-show="openModule === {{ $key }}" x-collapse
                                        class="bg-gray-800 dark:bg-black-900">
                            ...
                            {{ $completed ? 'bg-green-500 text-white border-green-600' : 'bg-gray-200 text-gray-200 border-gray-400' }}">
                                                        {{ $index + 1 }}
                                                    </div>

                                                    <!-- Título da aula -->
                                                    <a href="{{ route('student.classroom.show', ['uuid_classroom' => $classroom->uuid]) }}"
                                                        class="block text-sm font-medium text-gray-100 dark:text-gray-300 hover:text-indigo-500">
